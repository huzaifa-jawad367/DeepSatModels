...
def train_aug(imgs, mask, target):
    ...
    if k:
        imgs   = np.rot90(imgs, k, axes=(1, 2))  # rotate H, W axes
        target = np.rot90(target, k, axes=(0, 1))

    # small rotate + shift
    if random.random() > 0.3:
        d = int(imgs.shape[1] * 0.1)
        rot_pnt = (
            imgs.shape[1]//2 + random.randint(-d, d),
            imgs.shape[2]//2 + random.randint(-d, d)
        )
        angle = random.randint(-45, 45)
        if angle != 0:
            T, H, W, C = imgs.shape
            flat = imgs.reshape(H, W, T*C)
            flat = rotate_image(flat, angle, rot_pnt)
            imgs = flat.reshape(H, W, T, C).transpose(2, 0, 1, 3)  # [T, H, W, C]
